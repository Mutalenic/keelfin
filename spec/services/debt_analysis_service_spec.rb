  ...
  describe '#analyze' do
    ...
    context 'with debts under 40% threshold' do
      before do
        ...
        user.debts.create!(lender_name: 'Madison', principal_amount: 30_000, monthly_payment: 1500, status: 'active')
      end

      it 'calculates correct totals' do
        result = service.analyze

        expect(result[:total_debt]).to eq(80_000)
    ...
    context 'with debts over 40% threshold' do
      before do
        user.debts.create!(lender_name: 'Bayport', principal_amount: 50_000, monthly_payment: 3000, status: 'active')
        user.debts.create!(lender_name: 'Madison', principal_amount: 30_000, monthly_payment: 2000, status: 'active')
      end
    ...
    context 'payoff strategies' do
      before do
        user.debts.create!(lender_name: 'High Interest', principal_amount: 50_000, monthly_payment: 3000,
                           interest_rate: 25, status: 'active')
        user.debts.create!(lender_name: 'Low Interest', principal_amount: 30_000, monthly_payment: 2000,
                           interest_rate: 10, status: 'active')
        user.debts.create!(lender_name: 'Small Debt', principal_amount: 5000, monthly_payment: 500, interest_rate: 15,
                           status: 'active')
      end
