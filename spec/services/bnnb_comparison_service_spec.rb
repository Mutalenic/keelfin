...
RSpec.describe BnnbComparisonService do
  let(:user) do
    User.create!(name: 'Test User', email: 'test@example.com', password: 'password123', monthly_income: 15_000)
  end
  let(:category) { user.categories.create!(name: 'Food', icon: '🍔') }
  let(:month) { Date.current.beginning_of_month }
  let(:service) { described_class.new(user, month) }
  ...
  describe '#generate_insights' do
    ...
    it 'handles nil food_basket gracefully' do
      bnnb_data.update_columns(food_basket: nil) # rubocop:disable Rails/SkipsModelValidations
      bnnb_data.reload
    ...
    it 'handles zero food_basket gracefully' do
      bnnb_data.update_column(:food_basket, 0) # rubocop:disable Rails/SkipsModelValidations
      bnnb_data.reload
      result = service.send(:generate_insights, bnnb_data)
      expect(result).to eq([])
    end
