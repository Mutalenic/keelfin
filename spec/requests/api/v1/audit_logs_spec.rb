...
RSpec.describe 'Api::V1::AuditLogs', type: :request do
  ...
  describe 'GET /api/v1/audit_logs' do
    ...
    it 'returns balance_before, balance_after, and balance_delta' do
      ...
      log = JSON.parse(response.body)['audit_logs'].first
      expect(log.keys).to include('balance_before_cents', 'balance_after_cents', 'balance_delta_cents')
    end
    ...
    it 'does not expose another user\'s audit logs' do
      ...
      Ledger::TransactionProcessor.call(
        ...
        idempotency_key: SecureRandom.uuid,
        entries: [
          { account: other_wallet, direction: 'debit',  amount_cents: 1000 },
          { account: other_equity, direction: 'credit', amount_cents: 1000 }
        ]
      )
