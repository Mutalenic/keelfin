  ...
  describe 'POST /api/v1/ledger_transactions' do
    ...
    it 'creates a posted transaction and returns 201' do
      post '/api/v1/ledger_transactions',
           params: txn_params(
             ...
             entries: [
               { account_id: wallet.id,  direction: 'credit', amount_cents: 3000 },
               { account_id: expense.id, direction: 'debit',  amount_cents: 3000 }
             ]
           ),
           as: :json,
           headers: headers
    ...
    it 'returns 422 for imbalanced entries with a descriptive message' do
      post '/api/v1/ledger_transactions',
           params: txn_params(
             ...
             idempotency_key: SecureRandom.uuid,
             entries: [
               { account_id: wallet.id,  direction: 'credit', amount_cents: 5000 },
               { account_id: expense.id, direction: 'debit',  amount_cents: 3000 }
             ]
           ),
           as: :json,
      ...
      expect(JSON.parse(response.body)['error']).to match(/balance/i)
    end
    ...
    it 'returns 422 for insufficient funds' do
      post '/api/v1/ledger_transactions',
           params: txn_params(
             ...
             idempotency_key: SecureRandom.uuid,
             entries: [
               { account_id: wallet.id,  direction: 'credit', amount_cents: 999_999 },
               { account_id: expense.id, direction: 'debit',  amount_cents: 999_999 }
             ]
           ),
           as: :json,
           headers: headers
      ...
      expect(response).to have_http_status(:unprocessable_entity)
      expect(JSON.parse(response.body)['error']).to match(/insufficient/i)
    end
    ...
    it 'is idempotent — returns same transaction on duplicate key' do
      ...
      params = txn_params(
        ...
        idempotency_key: key,
        entries: [
          { account_id: wallet.id,  direction: 'credit', amount_cents: 1000 },
          { account_id: expense.id, direction: 'debit',  amount_cents: 1000 }
        ]
      )
