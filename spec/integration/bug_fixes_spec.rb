...
RSpec.describe 'Bug Fixes Integration Tests', type: :integration do
  ...
  describe 'Issue #4 & #5: Nil reference errors in views' do
    it 'handles nil interest_rate in debt view' do
      debt = user.debts.create!(lender_name: 'Bayport', principal_amount: 50000, interest_rate: nil)
    ...
    it 'handles nil monthly_payment in debt view' do
      debt = user.debts.create!(lender_name: 'Bayport', principal_amount: 50000, monthly_payment: nil)
  ...
  describe 'Issue #7: Status validation' do
    it 'accepts valid status values' do
      debt = user.debts.build(lender_name: 'Bayport', principal_amount: 50000, status: 'active')
    ...
    it 'rejects invalid status values' do
      debt = user.debts.build(lender_name: 'Bayport', principal_amount: 50000, status: 'invalid')
      expect(debt).not_to be_valid
      expect(debt.errors[:status]).to include('invalid is not a valid status')
    end
    
    it 'allows nil status' do
      debt = user.debts.build(lender_name: 'Bayport', principal_amount: 50000, status: nil)
      expect(debt).to be_valid
    end
  end
  ...
  describe 'Issue #2: Division by zero in BnnbComparisonService' do
    ...
    it 'handles zero food_basket gracefully' do
      bnnb = BnnbData.create!(month: Date.current.beginning_of_month, food_basket: 1000, non_food_basket: 1000, total_basket: 2000)
      bnnb.update_column(:food_basket, 0)
    ...
    it 'handles nil food_basket gracefully' do
      bnnb = BnnbData.create!(month: Date.current.beginning_of_month, food_basket: 1000, non_food_basket: 1000, total_basket: 2000)
      bnnb.update_columns(food_basket: nil)
  ...
  describe 'Issue #11: Inflation adjustment mutation' do
    ...
    it 'uses update! for atomic operation' do
      ...
      # Verify it uses update! (which is atomic)
      expect(budget).to receive(:update!).with(monthly_limit: 5500.0).and_call_original
  ...
  describe 'Issue #3: N+1 query optimization' do
    it 'caches active debts query in DebtAnalysisService' do
      user.debts.create!(lender_name: 'Bayport', principal_amount: 50000, status: 'active', interest_rate: 15.0)
      user.debts.create!(lender_name: 'Madison', principal_amount: 30000, status: 'active', interest_rate: 12.0)
  ...
  describe 'Issue #14: Improved error handling in ExchangeRateService' do
    it 'catches specific timeout errors' do
      ...
      expect(Rails.logger).to receive(:error).with(/Exchange rate API timeout/)
    ...
    it 'catches JSON parse errors' do
      ...
      expect(Rails.logger).to receive(:error).with(/Exchange rate JSON parse error/)
      expect { ExchangeRateService.fetch_latest_usd_zmw }.not_to raise_error
    end
