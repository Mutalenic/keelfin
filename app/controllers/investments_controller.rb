class InvestmentsController < ApplicationController
  ...
  def index
    ...
    @return_percentage = @total_initial.positive? ? ((@total_return / @total_initial) * 100).round(2) : 0
  ...
  def create
    ...
    if @investment.initial_amount.positive?
    ...
    transactions.group_by { |t| t.transaction_date.beginning_of_month }.map do |month, txns|
      {
        ...
        income: txns.select { |t| %w[dividend interest].include?(t.transaction_type) }.sum(&:amount),
