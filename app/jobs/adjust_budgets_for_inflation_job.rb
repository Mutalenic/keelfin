  ...
  def perform
    ...
    Budget.where(inflation_adjusted: true).find_each do |budget|
      if budget.adjust_for_inflation!(monthly_inflation)
        ...
        failed_count += 1
        Rails.logger.warn(
          "Failed to adjust budget #{budget.id}: #{budget.errors.full_messages.join(', ')}"
        )
      end
    end

    Rails.logger.info(
      "Adjusted #{adjusted_count} budgets for inflation (#{monthly_inflation}% monthly), " \
      "#{failed_count} failed"
    )
  rescue StandardError => e
    Rails.logger.error(
      "Failed to adjust budgets for inflation: #{e.message}\n#{e.backtrace.join('\n')}"
    )
    raise
  end
end
